# A Node.js blackcoin Client!


node-blackcoin is a blackcoin client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with most blackcoins. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with blackcoin integration.
* Promote Node.js development of blackcoin web apps.
* Identify and address any incompatibilities with the blackcoins and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of a [blackcoind](https://github.com/CoinBlack/blackcoin-more/releases/latest) to connect with. 

Then, install the node-blackcoin NPM package.

`npm install node-blackcoin`

or

`npm install git://github.com/CryptoLover705/node-blackcoin/`
...
blackcoin.auth('myusername', 'mypassword')

blackcoin.getDifficulty(function() {
...
```js
blackcoin.getNewAddress(function(err, address) {
...
```js
blackcoin.exec('getNewAddress')

blackcoin.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a blackcoin address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [blackcoinaddress] </td>
<td> Reveals the private key corresponding to <blackcoinaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current blackcoin address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether blackcoind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new blackcoin address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [blackcoinaddress] [minconf=1] </td>
<td> Returns the total amount received by <blackcoinaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute blackcoind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [toblackcoinaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<tr>
<td> signmessage </td>
<td> [blackcoinaddress] [message] </td>
...
<td> stop </td>
<td> </td>
<td> Stop blackcoin server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [blackcoinaddress] </td>
<td> Return information about [blackcoinaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [blackcoinaddress] [signature] [message] </td>
...
var blackcoin = require('blackcoin')({
    user:'user'
})

blackcoin.set('pass', 'somn')
blackcoin.set({port:15715})
...
You may pass an optional function `passphrasecallback` to the `node-blackcoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `blackcoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `blackcoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `blackcoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/blackcoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/blackcoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `blackcoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-blackcoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('blackcoind.crt')
